...
object WakeOnLanUtil {
    ...
    suspend fun sendWakeOnLan(
        ...
        withContext(Dispatchers.IO) {
            try {
                ...
                Log.d(TAG, "Magic packet sent to $macAddress via $targets")
                true
            } catch (e: Exception) {
    ...
    private fun getAllSubnetBroadcasts(): List<String> {
            ...
            for (networkInterface in interfaces) {
                ...
                for (interfaceAddress in networkInterface.interfaceAddresses) {
                    ...
                    if (broadcast != null && broadcast is Inet4Address) {
                        broadcasts.add(broadcast.hostAddress ?: continue)
                    }
                }
            }
        } catch (_: Exception) {
        }
        return broadcasts.ifEmpty { listOf("255.255.255.255") }
    }
